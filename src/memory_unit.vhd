...
ENTITY memory_unit IS
	PORT (
		...
		i_result_rd_ex : IN STD_LOGIC_VECTOR(31 DOWNTO 0); -- Would be Either Address or Passthrough
...
BEGIN
	...
	load_logic : PROCESS (i_mem_read_ex, i_funct3_ex, i_mem_read_data, i_result_rd_ex)
	...
	BEGIN
		...
		IF i_mem_read_ex = '1' THEN
			...
			CASE i_result_rd_ex(1 DOWNTO 0) IS
	...
	BEGIN
		...
		IF i_mem_write_ex = '1' THEN
			CASE i_funct3_ex IS
				...
				WHEN "001" => -- SH (Store Half)
					o_mem_write_data <= i_rs2_data_ex(15 DOWNTO 0) & i_rs2_data_ex(15 DOWNTO 0);
				...
				WHEN "000" => -- SB (Store Byte)
					o_mem_write_data <= i_rs2_data_ex(7 DOWNTO 0) & i_rs2_data_ex(7 DOWNTO 0) &
						i_rs2_data_ex(7 DOWNTO 0) & i_rs2_data_ex(7 DOWNTO 0);
	...
	o_result_rd_mem <= i_result_rd_ex;
	o_pc4_mem <= i_pc4_ex;
