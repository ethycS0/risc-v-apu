...
ENTITY execution_unit IS
	PORT (
		...
		i_result_mem : IN STD_LOGIC_VECTOR(31 DOWNTO 0);
...
BEGIN
	...
	WITH i_alu_src_b SELECT
		s_alu_input_b_id <= i_rs2_data WHEN ALU_B_RS2,
	...
	PORT MAP(
		...
		i_rd_addr_ex => i_rd_addr, -- The rd of the current instruction
		i_reg_write_ex => i_reg_write, -- The RegWrite of the current instruction
		i_rd_addr_mem => i_rd_addr_mem, -- The rd from the MEM stage
		i_reg_write_mem => i_reg_write_mem, -- The RegWrite from the MEM stage
		o_fwd_a_select => s_fwd_a_select,
		o_fwd_b_select => s_fwd_b_select
	);

	-- NEW: Forwarding MUX for ALU input A
	WITH s_fwd_a_select SELECT
		s_alu_input_a <= s_alu_input_a_id WHEN FWD_NONE, -- Use data from register file
		s_alu_result_raw WHEN FWD_FROM_EX, -- Forward result from this stage's ALU
		i_result_mem WHEN FWD_FROM_MEM; -- Forward result from MEM stage

	-- NEW: Forwarding MUX for ALU input B
	WITH s_fwd_b_select SELECT
		s_alu_input_b <= s_alu_input_b_id WHEN FWD_NONE,
		s_alu_result_raw WHEN FWD_FROM_EX,
		i_result_mem WHEN FWD_FROM_MEM;
