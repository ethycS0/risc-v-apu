...
ARCHITECTURE behavioral OF instruction_fetch_stage IS
	...
	-- Delayed PC/PC4 to align with registered instruction
	SIGNAL r_pc_delayed : STD_LOGIC_VECTOR(31 DOWNTO 0);
	SIGNAL r_pc4_delayed : STD_LOGIC_VECTOR(31 DOWNTO 0);
BEGIN
	o_instr_addr <= s_pc;

	-- Output delayed PC that matches the instruction timing
	...
	BEGIN
		...
		ELSIF rising_edge(i_clk) THEN
			IF i_stall = '0' THEN
				...
				-- Delay PC by 1 cycle to match registered IMEM output
				r_pc_delayed <= s_pc;
				r_pc4_delayed <= s_pc4;
			END IF;
		END IF;
