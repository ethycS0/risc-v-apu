...
BEGIN
	...
	BEGIN
		...
		IF (to_integer(unsigned(i_rd_addr_ex_mem)) /= 0) AND (i_rd_addr_ex_mem = i_rs1_addr_id) THEN
			o_fwd_a_select <= FWD_FROM_EX_MEM;

			-- Forward from Memory Stage
		ELSIF (to_integer(unsigned(i_rd_addr_mem_wb)) /= 0) AND (i_rd_addr_mem_wb = i_rs1_addr_id) THEN
			o_fwd_a_select <= FWD_FROM_MEM_WB;
		END IF;
		-- === Logic for Forwarding to ALU Input B (rs2) ===

		-- Forward from Execute Stage
		IF (to_integer(unsigned(i_rd_addr_ex_mem)) /= 0) AND (i_rd_addr_ex_mem = i_rs2_addr_id) THEN
			o_fwd_b_select <= FWD_FROM_EX_MEM;

			-- Forward from Memory Stage
		ELSIF (to_integer(unsigned(i_rd_addr_mem_wb)) /= 0) AND (i_rd_addr_mem_wb = i_rs2_addr_id) THEN
