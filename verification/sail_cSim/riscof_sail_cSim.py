...
class sail_cSim(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sail_config = os.path.join(
            self.pluginpath, config.get("sail_config", "rv32_config.json")
        )
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        if "D" in ispec["ISA"]:
            self.isa += "d"
        objdump = "riscv{0}-none-elf-objdump".format(self.xlen)
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            execute += self.sail_exe[
                self.xlen
            ] + " --config {0} --test-signature={1} {2} > {3}.log 2>&1;".format(
                self.sail_config, sig_file, elf, test_name
            )
